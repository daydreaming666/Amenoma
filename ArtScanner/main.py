...
while True:
    ...
    if len(windows) == 0:
        ...
        print('{0:^{4}} {1:^{5}} {2:^{6}} {3:^{7}}'.format('编号', '窗口标题', '进程名', '窗口内部分辨率',
                                                           calcFormatWidth('编号', 5), calcFormatWidth('窗口标题', 25),
                                                           calcFormatWidth('进程名', 25), calcFormatWidth('窗口内部分辨率', 15)))
        for i, window in enumerate(windows):
            print('{0:^5} {1:<{4}.{4}} {2:<{5}.{5}} {3:<15}'.format(i + 1, window[1], os.path.basename(window[2]),
                                                                    str(window[3]),
                                                                    calcFormatWidth(window[1], 25),
                                                                    calcFormatWidth(window[2], 25)))
...
try:
    while True:
        if art_scanner.stopped or not art_scanner.scanRows(rows=range(start_row, game_info.art_rows),
                                                           callback=artscannerCallback) or start_row != 0:
            break
        start_row = game_info.art_rows - art_scanner.scrollToRow(game_info.art_rows, max_scrolls=20,
                                                                 extra_scroll=int(game_info.art_rows > 5),
